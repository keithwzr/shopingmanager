...
const routes = [
  ...
  },
  {
    ...
    children: [
      ...
      { path: 'type', name: 'Type', meta: { name: '商品分类' }, component: () => import('../views/front/Type') },
...
//路由守卫
 router.beforeEach((to, from, next) => {
   ...
   if ((to.path === '/' || to.path === '/front/home') && user !== null && user.role === 'ADMIN'&& user.role === 'BUSINESS') {
     next('/home');
   } else if (to.path === '/front/home' && user !== null && user.role === 'USER') {
     next('/front/home');
   } else {
     next();
   }
